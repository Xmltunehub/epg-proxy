...
DEFAULT_OFFSET_SECONDS = 60  # +60 segundos global por defeito
...
def apply_offset_to_time(t_str, offset_seconds):
    dt, tz_str = parse_xmltv_time(t_str)
    if dt is None:
        return t_str
    dt_new = dt + timedelta(seconds=offset_seconds)
...
def fetch_and_process_epg():
    ...
    with update_lock:
        ...
        for programme in root.findall("programme"):
            ...
            offset = offsets.get(channel_id, DEFAULT_OFFSET_SECONDS)
...
def parse_m3u(content):
    """Extract channel names and tvg-id from M3U content"""
    channels = []
    ...
    for i, line in enumerate(lines):
        if line.startswith("#EXTINF"):
            ...
            name = tvg_name or display or tvg_id
            if name:
...
def api_channels():
    ...
    for ch in channels:
        ch["offset"] = offsets.get(ch["id"], DEFAULT_OFFSET_SECONDS)
...
def api_status():
    return jsonify({
        ...
        "default_offset": DEFAULT_OFFSET_SECONDS,
        "update_interval_hours": UPDATE_INTERVAL_HOURS
    })

@app.route("/api/m3u/fetch", methods=["POST"])
def api_fetch_m3u():
    """Fetch M3U from URL, extract channel names only — URL never stored"""
    data = request.json
    ...
    try:
        ...
        return jsonify({"ok": True, "channels": channels, "total": len(channels)})
    except Exception as e:
...
def api_set_mapping():
    """Save a mapping: m3u channel name -> epg channel id"""
    data = request.json
    m3u_name = data.get("m3u_name")
    epg_id = data.get("epg_id")
    if m3u_name is None or epg_id is None:
        return jsonify({"error": "Parâmetros inválidos"}), 400
    mappings = load_mappings()
    mappings[m3u_name] = epg_id
...
def api_delete_mapping():
    ...
    m3u_name = data.get("m3u_name")
    mappings = load_mappings()
    if m3u_name in mappings:
        del mappings[m3u_name]
        save_mappings(mappings)
    return jsonify({"ok": True})
